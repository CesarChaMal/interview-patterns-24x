...
function findMinHeightTrees(n: number, edges: number[][]): number[] {
    ...
    const leaves: number[] = [];
    for (let i = 0; i < n; i++) {
        if (graph.get(i)!.size === 1) leaves.push(i);
    }
    ...
    while (remaining > 2) {
        ...
        for (let i = 0; i < leafCount; i++) {
            ...
            if (graph.get(neighbor)!.size === 1) leaves.push(neighbor);
        }
    }
...
function sortItems(n: number, m: number, group: number[], beforeItems: number[][]): number[] {
    ...
    if (!itemOrder.length || !groupOrder.length) return [];
...
function buildMatrix(k: number, rowConditions: number[][], colConditions: number[][]): number[][] {
    function topSort(conditions: number[][]): number[] {
