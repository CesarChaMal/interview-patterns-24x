...
function solveNQueens(n: number): string[][] {
    ...
    const backtrack = (board: number[], row: number): void => {
        if (row === n) {
...
function solveSudoku(board: string[][]): void {
    const isValid = (row: number, col: number, c: string): boolean => {
        for (let i = 0; i < 9; i++) {
...
function combinationSum(candidates: number[], target: number): number[][] {
    const result: number[][] = [];
    const backtrack = (start: number, path: number[], sum: number): void => {
        if (sum === target) {
            result.push([...path]);
        } else if (sum < target) {
