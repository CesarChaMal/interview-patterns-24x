...
function stoneGameII(piles: number[]): number {
    ...
    for (let i = n - 2; i >= 0; i--) suffixSum[i] = suffixSum[i + 1] + piles[i];
...
function minDaysToEatOranges(n: number): number {
...
function maxProductPath(grid: number[][]): number {
    ...
    const dfs = (i: number, j: number): [number, number] => {
        if (i >= m || j >= n) return [Number.MIN_SAFE_INTEGER, Number.MIN_SAFE_INTEGER];
...
function numWaysToFormTarget(words: string[], target: string): number {
...
function maximumScore(nums: number[], multipliers: number[]): number {
    ...
    const dfs = (i: number, left: number): number => {
        if (i === m) return 0;
