...
function maxAreaOfIsland(grid: number[][]): number {
    const dfsArea = (i: number, j: number): number => {
...
function exist(board: string[][], word: string): boolean {
    const dfsSearch = (i: number, j: number, k: number): boolean => {
...
function solve(board: string[][]): void {
    ...
    const dfsMark = (i: number, j: number): void => {
    ...
    for (let i = 0; i < m; i++) {
        if (board[i][0] === 'O') dfsMark(i, 0);
        if (board[i][n-1] === 'O') dfsMark(i, n-1);
    }
    for (let j = 0; j < n; j++) {
        if (board[0][j] === 'O') dfsMark(0, j);
        if (board[m-1][j] === 'O') dfsMark(m-1, j);
    }
...
function pacificAtlantic(heights: number[][]): number[][] {
    ...
    const dfsFlow = (visited: boolean[][], i: number, j: number, prevHeight: number): void => {
