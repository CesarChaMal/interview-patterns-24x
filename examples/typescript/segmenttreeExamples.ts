...
// 3. Range Module
class RangeModule {
    private ranges: Map<number, number> = new Map();
    ...
    removeRange(left: number, right: number): void {
        ...
        for (const [l, r] of this.ranges) {
            if (r > left && l < right) {
                this.ranges.delete(l);
                if (l < left) toAdd.push([l, left]);
                if (r > right) toAdd.push([right, r]);
            }
        }
...
class MyCalendarThree {
    private delta: Map<number, number> = new Map();
...
class NumMatrix {
    ...
    private sum(row: number, col: number): number {
        let sum = 0;
