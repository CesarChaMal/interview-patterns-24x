...
function numTeamsInArray(rating: number[]): number {
...
class MRUQueue {
    ...
    private findKth(k: number): number {
        ...
        while (left < right) {
            const mid = left + Math.floor((right - left) / 2);
            if (this.ft.query(mid) < k) left = mid + 1;
            else right = mid;
        }
        return left;
    }
...
function goodTriplets(nums1: number[], nums2: number[]): number {
    ...
    for (const value of arr) rightFt.update(value, 1);
...
function minInteger(num: string, k: number): string {
    ...
    for (let i = 0; i < n; i++) ft.update(i, 1);
